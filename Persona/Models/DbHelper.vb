...
Public Class DbHelper
    ...
    ' Método para obtener la conexión abierta, con manejo de errores
    Public Function GetConnectionOpen() As SqlConnection
        ...
        Return conn
    End Function
    ' Método para obtener una nueva conexión (sin abrir)
    Public Function GetConnection() As SqlConnection
        Return New SqlConnection(connectionString)
    End Function

    ' Método para ejecutar un comando SQL (INSERT, UPDATE, DELETE)
    Public Function ExecuteNonQuery(query As String, Optional parameters As List(Of SqlParameter) = Nothing, Optional isStoredProcedure As Boolean = False) As Integer
        If String.IsNullOrWhiteSpace(query) Then
            Throw New ArgumentException("La consulta no puede estar vacía.")
        End If

        Using conn As SqlConnection = GetConnectionOpen()
    ...
    ' Método para ejecutar una consulta SQL y devolver un DataTable
    Public Function ExecuteProcedure(query As String, Optional parameters As List(Of SqlParameter) = Nothing) As DataTable
        If String.IsNullOrWhiteSpace(query) Then
            Throw New ArgumentException("La consulta no puede estar vacía.")
        End If
        ...
        Using conn As SqlConnection = GetConnection()
            Using cmd As New SqlCommand(query, conn)
                ...
                If parameters IsNot Nothing Then
                    cmd.Parameters.AddRange(parameters.ToArray())
                End If
                ' Establecer el tipo de comando si es un procedimiento almacenado
                cmd.CommandType = CommandType.StoredProcedure
